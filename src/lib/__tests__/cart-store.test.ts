...
describe('cartSubtotal', () => {
  it('is zero for an empty cart', () => {
    expect(cartSubtotal({ vendor_id: null, items: [], packaging_fee: 5 })).toBe(0);
  });
  ...
  it('does not include the packaging fee', () => {
    const c = { vendor_id: 'v1', packaging_fee: 5, items: [line(40, 1)] };
    expect(cartSubtotal(c)).toBe(40);
  });
});

describe('cartCount', () => {
  it('sums quantities across lines', () => {
    const c = { vendor_id: 'v1', packaging_fee: 5, items: [line(10, 3), line(10, 2)] };
