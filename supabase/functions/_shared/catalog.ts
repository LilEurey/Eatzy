...
// every available menu item at an open stall, plus its vendor name. menu_items and vendors are
...
const SELECT =
  'id,name,name_th,price,image_url,ingredients,tags,category,allergens,is_halal,is_vegetarian,is_jay,vendor_id,vendors!inner(name,is_open)';
...
export async function getRankingCatalog<T>(
  ...
  const { data, error } = await client
    ...
    // Closed stalls' dishes can't be ordered (place_order raises vendor_closed),
    // so they don't belong in a recommendation.
